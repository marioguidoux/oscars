...
var CollapseTemplate = {
	...
	load : function(json) {
		...
		$.each(json.residus, function(i, val) {
			_this.draw(val.name, val.childs, undefined, val.url, val.score);
		});
	},

	draw : function(name, childs, panel, url, score) {
		//console.log(childs);
		...
		this.drawHeader(name,  $newPanel, numId, dataParentId, score);
		//manejamos los hijos
	...
	drawHeader : function(name,  $newPanel, numId, dataParentId, score) {
		...
		$newPanel.find(".badge").text(score);
	},
	...
	drawChildNodesArray : function(childs,  $newPanel, numId) {
		...
		$.each(childs, function(i, val) {
			...
			//si se cargan los hijos por ajax
					...
					jqxhr.done(function(dades) {
						...
						$.each(dades.residus, function(i, val) {
							if(val.url===undefined || val.url===null)	{
								$(panel).append("<li class='list-group-item'>"+val.name+" <span class='badge badge-primary badge-pill>2</span> </li>");
							}	else	{
				...
				$newPanel.find(".panel-body").append("<ul class='list-group'><li class='list-group-item d-flex justify-content-between align-items-center'>" + val.name +  "<span class='badge badge-" + val.status + "'>" + val.winner + "</span> </li></ul>");
			}
		});
